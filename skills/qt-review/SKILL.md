...
4. **`emitNotify` 是在哪个线程被调用的**（项目使用自研通知总线时适用）？设备数据多在 TcpServer/串口/QThread 的 worker 线程到达，若该处再 `emitNotify`，而下游用 `Qt::DirectConnection` 接收，则下游槽**直接在 worker 线程执行**——必须把这条调用链先画出来。见下方「服务通知总线」一节。

## 0. 项目专属/可选：服务通知总线 (connectNotify / emitNotify)

若骨科机器人项目的模块之间**不直接 connect**，而是经 `ServerBase` 这类自研事件总线解耦：`emitNotify(NotifyStruct{code, paramMap})` 发，`connectNotify(code, receiver, SLOT(...), type)` 收（常见定义位置如 `CoreLib/ServerBase.cpp` + `ServerNotify`）。底层通常是一个真实 Qt 信号 `notify(unsigned int, const NotifyStruct&)`。项目存在这套总线时再逐条对照：
...
- [ ] **连接类型默认 `Qt::AutoConnection`**：worker 线程 `emitNotify` → GUI 线程 receiver 时自动走 Queued（安全，投递到 GUI 事件循环）。但骨科机器人项目里常见显式写 `Qt::DirectConnection` 的模块（机械臂、动力工具、脚踏、升级工具等）。DirectConnection 下槽**永远在发射线程执行**——若 `emitNotify` 源头在控制板 TCP/串口 worker 线程，则这些模块的 `getNotifyMessage` 槽就在 worker 线程里跑，槽内任何访问 UI / 本模块 QObject 状态都是**跨线程违规**。每见一个 DirectConnection 的 `connectNotify`，必须回溯 `emitNotify` 的调用线程。
...
- [ ] **别复用成员 `NotifyStruct` 反复 emit**：典型反例是控制板模块的 `sendNotify` 复用成员 `notifyStruct`，就地改 `code/paramMap` 再 emit。该函数在 socket worker 线程被高频调用，同一对象被并发改写 + 下游 `Qt::DirectConnection` 槽就地读 → 跨线程数据竞争 / 读到半写状态。每次 `emitNotify` 应构造**局部** `NotifyStruct`，不要复用可变成员。
...
- [ ] **本项目几乎全是「子类化 QThread + 重写 `run()`」**（`mythread`、`FootSwitchMonitor`、`TrackingManager`、`OTSThread` 等），`run()` 内是手写 `while(flag){...}` 轮询循环。两个固定坑：
  ...
  - **`stop()` 里调 `exit()`/`quit()` 对手写 `while` 循环无效**：`exit()/quit()` 只能停 `exec()` 起的事件循环；`run()` 没有事件循环时它们什么都不做，线程只能靠标志位退出。常见于控制板/设备线程的 `stop()`。
- [ ] **`QTimer::start()` 必须在定时器所属线程调用，且 `moveToThread` 要先于 `start`**。典型反例是交互控制板初始化中，`m_heartTimer->start(2000)` 在主线程调用后**才** `m_heartTimer->moveToThread(m_thread)`——Qt 会告警 "Timers cannot be started from another thread"，定时器行为不确定；且 `timeout` 用 `Qt::DirectConnection` 连主线程对象的 `heartCheck` 槽，定时器在 worker 线程触发时槽就地在 worker 线程跑、跨线程读写心跳成员。正确顺序：先 `moveToThread` → 再在该线程内（如 worker 的入口槽里）`start`，或干脆把定时器留在 GUI 线程用 `AutoConnection`。
- [ ] **busy-loop 无 `sleep` 满速占核**：`run()` 写成 `while(true){ getXxx(); }` 没有任何 `msleep`/条件等待（常见于 OTS/跟踪采集线程），连接后满速轮询设备，单核打满 + 与消费侧抢锁。轮询循环必须有节流（`msleep`/`QWaitCondition::wait` 超时）。
...
- [ ] **`QWaitCondition::wait()` 必须配裸 `QMutex` 手动 lock**，禁止与 `std::unique_lock`/`QMutexLocker` 混用。反例：`std::unique_lock<QMutex> locker(m); m_waitCondition.wait(&m, 4000)`——`wait` 内部已对锁做 unlock/relock，`unique_lock` 析构又解锁一次 → 二次解锁 UB。要么全程裸 `QMutex` + 手动 lock/unlock，要么用 `QMutexLocker`（但仍要把裸锁地址传给 `wait`，且 `wait` 期间 locker 不能析构）。
- [ ] **`QSemaphore` 当互斥量用时 `acquire`/`release` 必须异常安全配对**。反例：OTS/跟踪采集模块用 `m_semaphore(1)` + `acquire(); ...; release();` 包临界区——中途 `return`/抛异常漏掉 `release`，下一帧 `acquire` 永久阻塞采集线程。要么用 `QMutexLocker`（RAII），要么确保所有出口都 `release`。
- [ ] **设备 IO 拆包（`readyRead` + `QByteArray`/`QDataStream`）**：`readyRead` 每次 `readAll()` 拿到的是任意切分的 TCP/串口流，帧头帧尾判定必须基于**跨次累积缓冲**；信任协议里的"长度字段"去 `mid()` 前要校验上界防越界；手写小端移位（`buf[1] << 8`）在 `char`/有符号类型上做会**符号扩展**，应先转 `uchar`/`uint8_t`。反例包括控制板 framing 与 `QByteToShort`、串口 worker 循环内用循环外算出的 `bodyLength` 移除多包。字节序优先用 `QDataStream::setByteOrder` 而非手写移位。深入并发/协议正确性转 [[cpp-concurrency-review]] 与 [[cross-platform-guard]]（字节序/符号扩展）。
...
- [ ] **动态增删控件：`layout->takeAt()` 只取出 `QLayoutItem`，不删 widget**。节段/方案切换重建按钮组时必须 `delete item->widget(); delete item;`（正例可参考 `SurgicalConfigWidget::refreshLevelButtons` 这类局部重建函数）——漏删 widget 即泄漏；反之 widget 已被 parent 树接管又手动 delete 则 double-free。重建后立刻重连 `connect` 是对的，但要确认没有 pending 的 `singleShot`/排队信号仍指向**已 delete 的旧控件**。
...
| 形状 | 在哪 | 风险 |
|---|---|---|
| `connectNotify(code, this, SLOT(...), Qt::DirectConnection)` | 机械臂/动力工具/脚踏/升级工具模块 | 槽在 `emitNotify` 发射线程执行；若源头是设备 worker 线程→跨线程访问 |
| `connect(socket, SIGNAL(dataProcess(...)), this, SLOT(...), Qt::DirectConnection)` 后 `socket->moveToThread(t)` | 控制板 TCP 服务模块 | socket 信号在 worker 线程发，DirectConnection 使槽也在 worker 线程跑 |
| `new QThread(this)` + `new Worker(this)` + `this->moveToThread(...)` | 交互控制板/串口管理模块 | parent 关系与线程归属错乱，见上「moveToThread 误用」 |
...
| `QTimer` 后 `timer->moveToThread(t)`，timeout 用 `Qt::DirectConnection` 连 GUI 对象槽 | 交互控制板模块 | 定时器回调在 worker 线程触发，槽却操作 GUI 侧状态 |
| `std::thread([=]{...}).detach()` 捕获 `this`/外部标志 | 控制板定时器/后台回调模块 | 分离线程生命周期失控；回调触碰 QObject 即跨线程 |
| `connect(sender, &Sig, [this,p]{...})` 无 context 接收者 | 分割/AI 服务模块 | sender 先于 lambda 析构→悬空；改用三参 context 重载 |
| `timer->start(...)` 后才 `timer->moveToThread(t)`，`timeout` 用 DirectConnection | 交互控制板模块 | 在非所属线程 start 定时器（告警）；回调在 worker 线程跑却读主线程成员 |
| `this->moveToThread(m_thread)` 把管理对象自身搬走（worker 是 this 的 child） | 串口管理模块 | manager 公有方法的 emit 投递线程被改写；`stop()` 里 `emit closePort` 紧跟 `quit()` 不 wait，串口未关线程已退 |
| 复用成员 `NotifyStruct` 就地改 `code/paramMap` 再 `emitNotify` | 控制板 `sendNotify` | worker 线程高频复用同一对象 + 下游 Direct 槽就地读 → 数据竞争/读半写态 |
| `std::unique_lock<QMutex>` 配 `QWaitCondition::wait(&m,...)` | OTS/跟踪采集线程 | wait 内部已 relock，unique_lock 析构再解锁一次 → 二次解锁 UB |
| `readyRead`→`readAll()` 后信任长度字段 `mid()`；`buf[1]<<8` 在 char 上移位 | 控制板 TCP/串口 worker | 半包粘包未跨次累积、长度越界、符号扩展 |
| proxy 行号未 `mapToSource` 直接访问 source；`setModel` 后用旧 `selectionModel()` | 病例管理表格模块 | 取错行；旧 selectionModel 指针失效 |
| `layout->takeAt(0)` 后未 `delete item->widget()`（动态重建按钮组） | `SurgicalConfigWidget.cpp` | widget 泄漏；或反向 double-free |
| 裸 `blockSignals(true)...false` 包回填，中途可能 return | `SurgicalPlanWidget.cpp` | 漏 `false` → 信号永久静默；应用 `QSignalBlocker` |
| `QDialog::exec()` 在按钮槽内，槽内再弹 `KMessageBox` | 病例管理/弹窗模块 | 嵌套事件循环重入；exec 返回前 `this` 可能已销毁 |

> 安全范例（可作为「应该怎么写」的对照）：语音/提示模块的 `Qt::QueuedConnection`、工具注册/配准界面的 `QMetaObject::invokeMethod(..., Qt::QueuedConnection)`、导航跟踪界面把 QTimer 留在 GUI 线程。
